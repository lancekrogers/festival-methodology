...
func TestFindFestivalRoot(t *testing.T) {
	// Create temp directory structure: /tmp/xxx/festivals/.festival/ + festival with fest.yaml
	tmpDir := t.TempDir()
	...
	// Create a festival with fest.yaml
	festivalDir := filepath.Join(festivalsDir, "active", "my-festival")
	if err := os.MkdirAll(festivalDir, 0755); err != nil {
		t.Fatalf("failed to create festival dir: %v", err)
	}
	festYAMLPath := filepath.Join(festivalDir, "fest.yaml")
	if err := os.WriteFile(festYAMLPath, []byte("version: 1.0\n"), 0644); err != nil {
		t.Fatalf("failed to create fest.yaml: %v", err)
	}

	// Create nested directory inside the festival (phase/sequence)
	nestedDir := filepath.Join(festivalDir, "001_PLANNING", "01_research")
	if err := os.MkdirAll(nestedDir, 0755); err != nil {
		t.Fatalf("failed to create nested dir: %v", err)
	}

	// Create another festival without fest.yaml
	noConfigFestival := filepath.Join(festivalsDir, "active", "no-config-festival")
	if err := os.MkdirAll(noConfigFestival, 0755); err != nil {
		t.Fatalf("failed to create no-config festival dir: %v", err)
	}
	...
	tests := []struct {
		...
		{
			name:      "finds root from festival directory with fest.yaml",
			startDir:  festivalDir,
			wantRoot:  festivalDir,
			wantError: false,
		},
		{
			name:      "finds root from nested directory inside festival",
			startDir:  nestedDir,
			wantRoot:  festivalDir,
			wantError: false,
		},
		{
			name:      "error when festival has no fest.yaml",
			startDir:  noConfigFestival,
			wantRoot:  "",
			wantError: true,
		},
		{
			name:      "error when at festivals root (not inside a festival)",
			startDir:  festivalsDir,
			wantRoot:  "",
			wantError: true,
		},
		{
			name:      "error when outside festivals tree",
			startDir:  tmpDir,
			wantRoot:  "",
			wantError: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := FindFestivalRoot(tt.startDir)
